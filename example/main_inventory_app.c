...
// 메뉴 액션 콜백
typedef void (*menu_action_f)(app_state_t* app);

// 메뉴 아이템 구조체
typedef struct {
    const char* _label;   // "[Q]uit"
    menu_action_f _action;  // Callback
    int           _x;       // Rendered X (Auto Calculated)
    int           _w;       // Rendered Width (Auto Calculated)
} menu_item_t;
...
struct app_state_s {
    ...
    int             _drag_target_idx;
    int             _drag_item_idx;
    cc_coord_t      _drag_offset;
...
static void _truncate_text( const char* src, int max_width, char* out_buf, size_t buf_len ) {
    ...
    while( len > 0 ) {
        ...
        char* last = temp + len;
        do { last--; len--; } while( len > 0 && ( (*last & 0xC0) == 0x80 ) );
        *last = '\0';
    }
...
static void _inv_draw( inventory_t* inv, cc_buffer_t* buf ) {
    ...
    for( int i = 0; i < inv->_items._count; ++i ) {
        ...
        char prefix[16]; snprintf( prefix, sizeof(prefix), "%d. ", i + 1 );
...
static void _app_apply_maximized( app_state_t* app ) {
    ...
    for( int k = 0; k < app->_inv_count; ++k ) {
        ...
        inv->_rect.x = x_pos; inv->_rect.y = 2; inv->_rect.w = part_w - 1; inv->_rect.h = size._rows - 3;
    }
}
...
static void _app_check_window_collision( app_state_t* app, int current_idx ) {
    ...
    for( int i = 0; i < app->_inv_count; ++i ) {
        ...
        if( _rect_intersects( curr->_rect, app->_inventories[i]._rect ) ) {
            curr->_is_red_border = true; break;
        }
    }
}
...
static void _act_quit(app_state_t* app) {
    app->_is_running = false;
}

static void _act_max(app_state_t* app) {
    if( app->_view_mode == VIEW_NORMAL ) {
...
static void _act_restore(app_state_t* app) {
...
static void _add_menu(app_state_t* app, const char* label, menu_action_f act) {
    if (app->_menu_count < MAX_MENUS) {
        app->_menus[app->_menu_count]._label = label;
        app->_menus[app->_menu_count]._action = act;
        app->_menu_count++;
    }
}
...
static void _handle_mouse_press( app_state_t* app, int mx, int my ) {
    // Top Bar (Menu) Check
    if( my == 0 ) {
        // Data-driven menu hit test
        for( int i = 0; i < app->_menu_count; ++i ) {
            menu_item_t* m = &app->_menus[i];
            if( mx >= m->_x && mx < m->_x + m->_w ) {
                if( m->_action ) m->_action(app);
                return;
            }
        }
        return;
...
static void _handle_mouse_drag( app_state_t* app, int mx, int my ) {
    if( app->_drag_mode == DRAG_WINDOW_MOVE ) {
        ...
        rect_t test = inv->_rect; test.w = new_w; test.h = new_h;
        if( !_app_check_collision( app, app->_drag_target_idx, test ) ) {
            inv->_rect.w = new_w; inv->_rect.h = new_h;
        }
    }
        ...
        for( int i = 0; i < app->_inv_count; ++i ) {
            ...
            if( _rect_contains( check, mx, my ) ) app->_inventories[i]._is_green_border = true;
        }
    }
}
...
static void _handle_mouse_release( app_state_t* app ) {
    if( app->_drag_mode == DRAG_WINDOW_MOVE ) {
        ...
        if( inv->_rect.x == app->_drag_start_win_pos._x && inv->_rect.y == app->_drag_start_win_pos._y ) {
            ...
            inv->_rect.x = cx; inv->_rect.y = cy;
            inv->_is_red_border = _app_check_collision( app, app->_drag_target_idx, inv->_rect );
            if( inv->_is_red_border ) {
                cc_coord_t valid = _app_find_valid_pos( app, app->_drag_target_idx );
                inv->_rect.x = valid._x; inv->_rect.y = valid._y;
                inv->_is_red_border = false;
            }
        }
    }
...
static void _process_input( app_state_t* app, const cc_input_event_t* ev ) {
    if( ev->_code == CC_KEY_MOUSE_EVENT ) {
        app->_mouse_cursor._x = ev->_data._mouse._x;
        app->_mouse_cursor._y = ev->_data._mouse._y;
    }

    if( ev->_code == CC_KEY_q ) { app->_is_running = false; return; }
    if( ev->_code == CC_KEY_F1 ) { _act_max(app); }
    if( ev->_code == CC_KEY_F2 ) { _act_restore(app); }

    if( ev->_code == CC_KEY_MOUSE_EVENT ) {
        int mx = app->_mouse_cursor._x;
        int my = app->_mouse_cursor._y;
        if( ev->_data._mouse._button == CC_MOUSE_BTN_LEFT ) {
            if( ev->_data._mouse._action == CC_MOUSE_ACTION_PRESS ) _handle_mouse_press( app, mx, my );
        }
        if( ev->_data._mouse._action == CC_MOUSE_ACTION_DRAG ) _handle_mouse_drag( app, mx, my );
        else if( ev->_data._mouse._action == CC_MOUSE_ACTION_RELEASE ) _handle_mouse_release( app );
    }
}
...
static void _render( app_state_t* app ) {
    ...
    for( int i = 0; i < app->_menu_count; ++i ) {
        ...
        // Calculate and Store Hitbox
        int w = (int)cc_util_get_string_width( m->_label );
        m->_x = cx;
        m->_w = w;

        cc_buffer_draw_string( app->_screen_buffer, cx, 0, m->_label, &white, &blue );
        cx += w;

        if( i < app->_menu_count - 1 ) {
    ...
    char log_buf[512]; snprintf( log_buf, sizeof(log_buf), " Log: %s", app->_log_msg );
    ...
    if( app->_drag_mode == DRAG_ITEM_MOVE ) {
        ...
        char content[MAX_TEXT_LEN + 2]; snprintf( content, sizeof(content), " %s ", app->_dragging_item.name );
        int w = (int)cc_util_get_string_width( content );
        cc_color_t box_bg = CC_COLOR_BLACK; cc_color_t box_fg = CC_COLOR_CYAN;
...
void app_init(app_state_t* app) {
    ...
    // Menu Registration
    app->_menu_count = 0;
    _add_menu( app, " [Q]uit ",    _act_quit );
    _add_menu( app, " [F1] Max ",  _act_max );
    _add_menu( app, " [F2] Norm ", _act_restore );
    ...
    int total_cnt = 0;
    for( int i = 0; i < app->_inv_count; ++i ) {
        char title[64]; snprintf( title, sizeof(title), "Inventory %c", 'A' + i );
